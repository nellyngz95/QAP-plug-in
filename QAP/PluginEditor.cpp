    ...
    : AudioProcessorEditor (&p), audioProcessor (p),thumbnail(512, audioProcessor.formatManager, thumbnailCache)
{
    ...
    loadLibraryButton.setColour(juce::TextButton::buttonColourId, juce::Colours::darkorange);
    ...
    searchBar.setTextToShowWhenEmpty("Search sounds...", juce::Colours::lightgoldenrodyellow);
    searchBar.setColour(juce::TextEditor::focusedOutlineColourId, juce::Colours::orange);
    searchBar.setColour(juce::TextEditor::textColourId, juce::Colours::orange);
    ...
    recordButton.setColour(juce::TextButton::buttonColourId, juce::Colours::red); // Stop color
    recordButton.setColour(juce::TextButton::buttonOnColourId, juce::Colours::green); // Start color
...
{
    int y = 20;
    const int recordButtonWidth = 120;
    const int recordButtonHeight = 35;

    const int buttonWidth = 120;
    const int buttonHeight = 35;
    const int gap=10;
    const int combinedWidth = recordButtonWidth + buttonWidth + gap;
    int groupX = (getWidth() / 2) - (combinedWidth / 2);
    loadLibraryButton.setBounds(groupX,y,buttonWidth,buttonHeight);
    recordButton.setBounds(loadLibraryButton.getRight() + gap,y,recordButtonWidth,recordButtonHeight);
    ...
    if (explosionMode)      proceduralPanelWidth = 250;
    else if (FireMode)      proceduralPanelWidth = 250;
    else if (GunMode)       proceduralPanelWidth = 250;
    else if (JetMode)       proceduralPanelWidth = 250;
    else if (HelicopterMode)proceduralPanelWidth = 250;
    else if (RocketMode)    proceduralPanelWidth = 250;
    ...
    const int waveformMargin = 20;
    const int waveformY = getHeight() - waveformHeight - waveformMargin;

    wavFileList.setBounds(10, y, getWidth() - 20 - rightPanelReserve, waveformY - y);

    waveformBounds = juce::Rectangle<float>(
        10.0f,
        (float)waveformY,
        getWidth() - 20.0f - rightPanelReserve,
        (float)waveformHeight
    );

    // === Assistant (image + label) ===
...
void QAPAudioProcessorEditor::updateAssistantForParameter(const juce::String& paramID)
{
    ...
    {
        assistantLabel.setText(
            "How fast or slow the rumble will dissapear.",
            juce::dontSendNotification);
    }
    else if (paramID == "air")
    {
        assistantLabel.setText(
            "Controls the high frequency shockwave.",
            juce::dontSendNotification);
    }
    else if (paramID == "airDecay")
    {
        assistantLabel.setText(
            "How fast the air blast fades.",
            juce::dontSendNotification);
    }
    else if (paramID == "dust")
    {
        assistantLabel.setText(
            "Mid-frequency layer, for debris after the explosion.",
            juce::dontSendNotification);
    }
    else if (paramID == "dustDecay")
    {
        assistantLabel.setText(
            "Sets how long the dust and debris lasts.",
            juce::dontSendNotification);
    }
    ...
    {
        assistantLabel.setText(
            "Controls the motion of the flames. Try <0.70.",
            juce::dontSendNotification);
    }
    else if (paramID == "hissing")
    {
        assistantLabel.setText(
            "Is the steam escaping from the burning material. Try <0.50.",
            juce::dontSendNotification);
    }
    else if (paramID == "crackling")
    {
        assistantLabel.setText(
            "Controls the sharp pops from the material.Try <0.50.",
            juce::dontSendNotification);
    }
    else if (paramID == "intensity")
    {
        assistantLabel.setText(
            "Overall energy of the fire. ",
            juce::dontSendNotification);
    }
    //GUN
    ...
    {
        assistantLabel.setText(
            "Higher values create denser impacts.",
            juce::dontSendNotification);
    }
    else if (paramID == "shellfreqdecay")
    {
        assistantLabel.setText(
            "How quickly the shell impacts fade away.",
            juce::dontSendNotification);
    }
    ...
    {
        assistantLabel.setText(
            "Controls speed and movement, intensity of engine.",
            juce::dontSendNotification);
    }
    else if (paramID == "turbine")
    {
        assistantLabel.setText(
            "High-pitched mechanical sound from the jet engines.",
            juce::dontSendNotification);
    }
    else if (paramID == "burn")
    {
        assistantLabel.setText(
            "Controls the combustion roar.Adds heat and intensity.",
            juce::dontSendNotification);
    }
    ...
    {
        assistantLabel.setText(
            "Controls the timing between each blade pass.",
            juce::dontSendNotification);
    }
    else if (paramID == "period")
    {
        assistantLabel.setText(
            "Acts like a timing reference for the rotor. Start with 149.",
            juce::dontSendNotification);
    }
    else if (paramID == "tailMix")
    {
        assistantLabel.setText(
            "Higher values emphasize the high-pitched buzzing.",
            juce::dontSendNotification);
    }
    else if (paramID == "baseFreq")
    {
        assistantLabel.setText(
            "Controls the sound of the helicopters main sound.Lower=heavy.",
            juce::dontSendNotification);
    }
    else if (paramID == "rotorMix")
    {
        assistantLabel.setText(
            "Makes the signature 'whup-whup' sound more pronounced or not.",
            juce::dontSendNotification);
    }
    else if (paramID == "engineMix")
    {
        assistantLabel.setText(
            "Balances the engine noise relative to the rotor sound.",
            juce::dontSendNotification);
    }
    else if (paramID == "bladeNoise")
    {
        assistantLabel.setText(
            "Controls the air rush and 'slap' from rotor tips.",
            juce::dontSendNotification);
    }
    else if (paramID == "engineSpeed")
    {
        assistantLabel.setText(
            "Higher values increase overall intensity and pitch.",
            juce::dontSendNotification);
    }
    ...
    {
        assistantLabel.setText(
            "Adjusts the flutter or instability in the rocket exhaust.",
            juce::dontSendNotification);
    }
