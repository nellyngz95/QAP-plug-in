    ...
    : AudioProcessorEditor (&p), audioProcessor (p),thumbnail(512, audioProcessor.formatManager, thumbnailCache)
{
    ...
    //rumbleAttachment.reset(new juce::AudioProcessorValueTreeState::SliderAttachment(audioProcessor.parameters, "rumble_decay", rumbleSlider));

    // Set up the air slider
    addAndMakeVisible(airSlider);
    airSlider.setSliderStyle(juce::Slider::LinearHorizontal);
    airSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, 60, 20);
   // airAttachment.reset(new juce::AudioProcessorValueTreeState::SliderAttachment(audioProcessor.parameters, "air_decay", airSlider));
    addAndMakeVisible(triggerButton);
    triggerButton.setButtonText("Trigger Explosion");
    triggerButton.onClick = [this]() { audioProcessor.triggerExplosion();};

    setSize(500, 400); // Set the overall size of your plugin editor
}
...
void QAPAudioProcessorEditor::setupExplosionUI()
{
    ...
    // Remember to set up and add any other sliders and their attachments here
...
void QAPAudioProcessorEditor::updateAssistant(const juce::String& searchText)
{
    if (searchText.containsIgnoreCase("explosion"))
    {
        ...
        //audioProcessor.triggerExplosion();
