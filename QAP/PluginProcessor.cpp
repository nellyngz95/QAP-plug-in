...
void QAPAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    auto& lowShelf = eqChain.template get<0>();
    auto& peakFilter = eqChain.template get<1>();
    auto& highShelf = eqChain.template get<2>();
    lowShelf.coefficients = juce::dsp::IIR::Coefficients<float>::makeLowShelf(
        sampleRate,
        100.0f,     // Frequency (Hz)
        0.707f,     // Q factor
        juce::Decibels::decibelsToGain(3.0f) // Gain
    ); // <--- Removed the '*'
    ...
    peakFilter.coefficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(
        ...
        1500.0f,
        1.5f,
        juce::Decibels::decibelsToGain(-6.0f) // Gain
    );
    ...
    highShelf.coefficients = juce::dsp::IIR::Coefficients<float>::makeHighShelf(
        ...
        8000.0f,
        0.707f,
        juce::Decibels::decibelsToGain(4.0f) // Gain
    );
